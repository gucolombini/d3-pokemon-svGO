...
// const centre = trapinch.node().getBBox();
...
fetch(trapinchFile)
    ...
    .then(data => {
        ...
        // d3.select(trapinch).attr("viewBox", `-50 -50 200 200`)
...
function loadPokeball() {
    fetch("assets/pokeball.svg")
        ...
        .then(data => {
            ...
            group.node().appendChild(wrapper); // Append <g> to the main group
...
function throwBall(ball, lastPosX, lastPosY) {
    ...
    function stepCounter() {
        currentX = currentX - stepX + deltaXMean*((stepAmount/2)-currSteps)/1000;
        velocityY += gravity;
        currentY += velocityY;
            ...
            .on("end", function() {
                ...
                if (currSteps < stepAmount) {
                    ...
                    ball.transition()
                        ...
                        .transition()
                        .duration(1000)
                        .attr("transform", `translate(${currentX}, ${140})`)
    ...
    function catchSequence(currentX, shakeCount) {
        ...
        const fleeChance = 0.8 // 30% per shake
        d3.select(pokeball).select("#ball").transition()
